...
echo '<style>' . "\n" . '    td.px-4.py-3 {' . "\n" . '    max-width: 240px;' . "\n" . '    text-wrap: pretty;' . "\n" . '}' . "\n" . '    </style>' . "\n" . '<main class="h-full pb-16 overflow-y-auto">' . "\n" . '    <div class="container grid px-6 mx-auto">' . "\n" . '        <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">Afiliados' . "\n" . '        <a href="./?page=affiliates/create_affiliate" id="create_new"><button class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">' . "\n\t\t\t" . 'Cadastrar afiliado' . "\n\t\t" . '</button></a>' . "\n" . '        <a href="./?page=affiliates/create_payment" id="create_new"><button class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">' . "\n\t\t\t" . 'Cadastrar pagamento' . "\n\t\t" . '</button></a>' . "\n" . '        </h2>' . "\n\n" . '        <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3 mb-4">' . "\n" . '        <!-- Card -->' . "\n" . '        <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">' . "\n" . '            <div class="p-3 mr-4 text-green-500 bg-green-100 rounded-full dark:text-green-100 dark:bg-green-500">' . "\n" . '            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">' . "\n" . '                <path' . "\n" . '                d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z">' . "\n" . '                </path>' . "\n" . '            </svg>' . "\n" . '            </div>' . "\n" . '            <div>' . "\n" . '            <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">' . "\n" . '                Afiliados ativos' . "\n" . '            </p>' . "\n" . '            <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">' . "\n" . '            ';
...
while ($row = $requests->fetch_assoc()) {
	...
	echo '</td>' . "\n" . '                    <td class="px-4 py-3">' . "\n" . '                        <div class="flex items-center space-x-2 text-sm">' . "\n" . '                            <a href="./?page=affiliates/create_affiliate&id=';
	echo $row['id'];
...
if (0 < $totalPages) {
	...
	if (3 < $page) {
		echo '            <a href="./?page=affiliates&pg=1"><li><button class="px-3 py-1 rounded-md focus:outline-none focus:shadow-outline-purple">1</button></li></a>' . "\n" . '            <li class="dots">...</li>' . "\n" . '        ';
	}

	echo "\n" . '        ';

	if (0 < ($page - 2)) {
		echo '            <a href="./?page=affiliates&pg=';
		echo $page - 2;
