...
// Verifica se os parâmetros id e page estão definidos na URL
if (!isset($_GET['id'])) {
?>

<div style="margin-bottom: 20px; display: flex; gap: 10px; justify-content: flex-end;">
    <a href="<?php echo base_url ?>admin/?page=cotas/gerar" 
...
<form style="margin-top: 20px; background-color: #f5f5f5; padding: 20px; border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);" id="rifaForm" action="<?php echo 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; ?>" method="get">
    ...
    <select id="rifas" name="id" style="width: 100%; padding: 10px; border-radius: 5px; border: 1px solid #ccc; background-color: #fff; color: #333;">
        <option value="">Selecione...</option>
        <?php
        // Consulta SQL para selecionar todas as rifas
        $sql = "SELECT id, name FROM product_list";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // Exibe cada rifa encontrada
    ...
    <br><br>
    <button type="submit" style="background-color: #ffc107; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; transition: background-color 0.3s ease;">Ver ganhadores</button>
</form>
...
<?php
    ...
    if (isset($_GET['id'])) {
        ...
        function getCustomerDataById($customerId)
        {
            ...
            $sql = "SELECT * FROM customer_list WHERE id = $customerId";
            $result = $conn->query($sql);
    ...
    if (!empty($dezenas_busca)) {
      ...
      if ($result_comprador && $result_comprador->num_rows > 0) {
        ...
        $customerData = getCustomerDataById($row_comprador['customer_id']);
            $cli =  $customerData['firstname'] . ' ' . $customerData['lastname'];}else{
