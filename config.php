...
function drope_gn_setwebhook($tokenType, $client_chave_pix, $accessToken, $api_pix_certificate)
{
	...
	$curl = curl_init();
	curl_setopt_array($curl, [
		CURLOPT_URL => 'https://pix.api.efipay.com.br/v2/webhook/' . $client_chave_pix,
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => '',
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 0,
...
function drope_gn_get_qrcode($loc_id, $tokenType, $accessToken, $api_pix_certificate)
{
	$curl = curl_init();
	curl_setopt_array($curl, [
		CURLOPT_URL => 'https://pix.api.efipay.com.br/v2/loc/' . $loc_id . '/qrcode',
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => '',
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 0,
...
function gerencianet_generate_pix($order_id, $amount, $client_name, $client_email, $order_expiration)
{
	...
	$pix_url_cob = 'https://pix.api.efipay.com.br/v2/cob/' . $txID;
...
function decode_brcode($brcode)
{
	...
	while ($n < strlen($brcode)) {
		$codigo = substr($brcode, $n, length: 2);
		$n += 2;
...
function drope_paggue_get_info($info)
{
	...
	curl_setopt_array($curl, [CURLOPT_URL => 'https://ms.paggue.io/payments/api/auth/login', CURLOPT_RETURNTRANSFER => true, CURLOPT_ENCODING => '', CURLOPT_MAXREDIRS => 10, CURLOPT_TIMEOUT => 0, CURLOPT_FOLLOWLOCATION => true, CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1, CURLOPT_CUSTOMREQUEST => 'POST', 0 => curl_setopt($curl, CURLOPT_POSTFIELDS, $data)]);
	$response = curl_exec($curl);
	$get = json_decode($response, true);
	curl_close($curl);
...
function drope_paggue_create_order($client_name, $order_item, $amount, $order_id)
{
	global $_settings;
	$curl = curl_init();
	$data = ['payer_name' => $client_name, 'amount' => $amount, 'external_id' => $order_id, 'description' => $order_item];
	$signature = hash_hmac('sha256', json_encode($data), $_settings->info('paggue_client_secret'));
	$headers = ['Accept: application/json', 'Content-Type: application/json', 'Authorization: Bearer ' . drope_paggue_get_info('access_token'), 'X-Company-ID: ' . drope_paggue_get_info('company_id'), 'Signature: ' . $signature];
	curl_setopt_array($curl, [CURLOPT_URL => 'https://ms.paggue.io/payments/api/billing_order', CURLOPT_RETURNTRANSFER => true, CURLOPT_ENCODING => '', CURLOPT_MAXREDIRS => 10, CURLOPT_TIMEOUT => 0, CURLOPT_FOLLOWLOCATION => true, CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1, CURLOPT_CUSTOMREQUEST => 'POST', CURLOPT_POSTFIELDS => json_encode($data), CURLOPT_HTTPHEADER => $headers]);
	$response = curl_exec($curl);
	$get = json_decode($response, true);
	curl_close($curl);
...
function nextpay_generate_pix($order_id, $amount, $client_name, $client_email, $order_expiration)
{
    ...
    $next_client_id = $_settings->info('nextpay_client_id');
    $next_client_secret = $_settings->info('nextpay_client_secret');
    ...
    $sql = "UPDATE order_list
    SET payment_method = '{$payment_method}', pix_code = '{$pix_code}', pix_qrcode = '{$pix_qrcode}',txid = '{$codePIXID}', order_expiration = '{$order_expiration}'
    WHERE id = {$order_id}";
...
function paggue_generate_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration)
{
	...
	$sql = 'UPDATE order_list' . "\r\n" . '        SET payment_method = \'' . $payment_method . '\', pix_code = \'' . $pix_code . '\', pix_qrcode = \'' . $pix_qrcode . '\', order_expiration = \'' . $order_expiration . '\', id_mp = \'' . $hash . '\'' . "\r\n" . '        WHERE id = ' . $order_id;
...
// FUNÇÃO DE ENVIO DE EMAIL COMENTADA - NÃO UTILIZADA NO DIA A DIA
...
if (!defined('APP_NAME')) {
	define('APP_NAME', 'Rifas Supremium');
}
...
if (!defined('DEV_NAME')) {
	define('DEV_NAME', 'Rifas Premium');
}

if (!defined('DEV_URL')) {
	define('DEV_URL', 'https://rifassupremium.com.br');
}

if (!defined('SUPPORT_URL')) {
	define('SUPPORT_URL', 'https://rifassupremium.com.br');
}
